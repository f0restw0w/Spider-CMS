...
function m__edit() {
	...
	if (intval ( $user ['uid'] ) > 0) {
		...
		if (isset ( $user ['upass'] )) { // 如果设置了密码，就是要改密码
			if (strlen ( $user ['new_upass'] ) < 5)
				die ( '{"code":"230","msg":"密码过于简单"}' );
			if (strlen ( $user ['new_upass'] ) > 20)
				die ( '{"code":"240","msg":"密码超出限定长度"}' );
				// 判断是否是当前账号的登陆状态
			if ($user ['uid'] != $_SESSION ['uid'])
				die ( '{"code":"250","msg":"不能修改其他账号的密码"}' );
				// 判断原始密码是否正确
			$chu = $dbm->single_query ( array (
					"where" => $where,
					'table_name' => TB_PREFIX . "admin_list" 
			) );
			if (helper::password_encrypt ( $user ['upass'] ) != $chu ['list'] [0] ['upass'])
				die ( '{"code":"260","msg":"旧密码错误"}' );
			if ($user ['new_upass'] != $user ['re_upass'])
				die ( '{"code":"270","msg":"两次密码不一样"}' );
		...
		die ( '{"code":"280","msg":"编辑账号失败，请核实后再添加"}' );
	         ...
	         // 验证注册信息是否合法
		if (empty ( $user ['uname'] ))
			die ( '{"code":"210","msg":"账号名不能为空"}' );
		if (empty ( $user ['upass'] ))
			die ( '{"code":"220","msg":"密码不能为空"}' );
		if (! preg_match ( '~^[A-Za-z][A-Za-z]*[a-z0-9_]*$~', $user ['uname'] ))
			die ( '{"code":"230","msg":"账号名必须以字母开头，只允许字母、数字、下划线"}' );
		if (strlen ( $user ['upass'] ) < 5)
			die ( '{"code":"240","msg":"密码过于简单"}' );
		if (strlen ( $user ['upass'] ) > 20)
			die ( '{"code":"250","msg":"密码超出限定长度"}' );
		if ($user ['upass'] != $user ['re_pass'])
			die ( '{"code":"270","msg":"两次密码不一致"}' );
		$where = " uname='" . $user ['uname'] . "'";
		$a = $dbm->single_query ( array (
				'where' => $where,
				'table_name' => TB_PREFIX . "admin_list" 
		) );
		if (count ( $a ['list'] ) > 0)
			die ( '{"code":"260","msg":"账号名不能重复"}' );
