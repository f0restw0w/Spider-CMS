...
function m__downall() {
	...
	if (isset ( $_GET ['start'] ) && ! empty ( $_GET ['start'] )) {
		if (! is_numeric ( $_GET ['start'] ))
			die ( '<font color=red>开始页码必须是数字</font>' );
	...
	if (isset ( $_GET ['end'] ) && ! empty ( $_GET ['end'] )) {
		if (! is_numeric ( $_GET ['end'] ))
			die ( '<font color=red>结束页码必须是数字</font>' );
		$end = ( int ) $_GET ['end'];
		$url = "&end=$end";
	} else {
		$url = '';
	...
	if (! empty ( $start ) && ! empty ( $end )) {
		if ($start > $end)
			die ( '<font color=red>开始页码必须小于结束页码</font>' );
	}
...
function m__auto_start() {
	...
	if (connection_status () != CONNECTION_NORMAL) {
		...
		while ( file_exists ( $flag_file ) ) {
			...
			if (! file_exists ( $flag_file ))
				break; // 标识文件不存在立即终止任务（任务延时未执行，已停止任务的情况）
			$next_time = 60;
			// 请求数据接口
...
function addapp($app) {
	...
	$resapp = $dbm->single_query ( array (
			'table_name' => TB_PREFIX . 'app_list',
			'where' => $where 
	) );
...
function insert_history($history, $resources, $appid) {
	...
	$hisres = $dbm->single_query ( array (
			'table_name' => TB_PREFIX . 'app_history',
			'where' => $where 
	) );
