...
function m__edit() {
	...
	if (! is_numeric ( $page ['post'] ['area_id'] ))
		die ( '{"code":"210","msg":"推荐位ID必须是数字"}' );
	if (empty ( $page ['post'] ['title'] ))
		die ( '{"code":"100","msg":"标题不能为空"}' );
	if ($page ['post'] ['area_type'] > 2) {
		if (! isset ( $page ['post'] ['area_type'] ))
			die ( '{"code":"200","msg":"推荐类型不能为空"}' );
		if (! is_numeric ( $page ['post'] ['area_order'] ))
			die ( '{"code":"200","msg":"排序必须是数字"}' );
	}
	if (! isset ( $page ['post'] ['area_id'] ) || $page ['post'] ['area_id'] <= 0) {
		// 添加推荐位或者广告
